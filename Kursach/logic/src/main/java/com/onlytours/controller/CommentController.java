...
import com.onlytours.service.CommentService;
...
//        commentDto.setUsername(account.getName());
        ...
        Optional<Comment> previousComment = commentRepository.findByAccount_IdAndTourId(account.getId(), commentDto.getTourId());
    ...
    @Transactional
    @ResponseBody
    public ResponseEntity loadComments(@RequestParam Long tourId, @RequestParam int pageNumber, @RequestParam(defaultValue = "7") int pageSize) {

        Pageable pageable = PageRequest.of(pageNumber, pageSize);
        List<Comment> comments = commentRepository.findAllByTourId(tourId, pageable);
        if (!comments.isEmpty()) {

...
<head>
    ...
    <script src="/resources/jquery/login.js"></script>
        ...
        <div class="modal-dialog formContentDelete">
            <div>
                ...
                <div class="modal-body">
                    <form id="errorForm">
                        <label class="questionForm"><spring:message code="unexpected.error"/></label>
                    </form>
                </div>
            </div>
        </div>
            ...
            <div class="modal-body">
                <form id="loginForm" action="/j_spring_security_check" method="post">
                    ...
                    <c:if test="${not empty error}">
                        <div id="loginMistake"><p class='alert alert-danger' role='alert'><spring:message code="${error}"/></p></div>
                    </c:if>
    ...
    <nav class="navbar navbar-expand-md  fixed-top">
        <div class="container">
            <div id="navbar" class="collapse navbar-collapse">
                ...
                <div id="buttons" class="position-1"></div>
                <label for="locales"></label><select id="locales" class="mdb-select md-form position-5">
                <option value=""></option>
                <option value="en"><spring:message code="lang.eng"/></option>
                <option value="ru"><spring:message code="lang.ru"/></option>
            </select>
                <div id="user" class="position-for-logBtns"></div>
                <div id="logoutBtn"></div>
                <sec:authorize access="isAnonymous()">
                    <script>
                        $('#user').append('<button type="button" class="btn btn-info position-button" id="signup" onclick="signUpForm()"><spring:message code="signup.button"/></button><button type="button" class="btn btn-info position-button" id="login" onclick="loginForm(0)"><spring:message code="signin.button"/></button>')
                    </script>
                </sec:authorize>
                <sec:authorize access="hasRole('ROLE_ADMIN')">
                    <script>

                        $('#buttons').append('    <a href="/showTours/lastOffer"  class="btn btn-outline-primary"><spring:message code="offer"/></a>\n' +
                            '                <a href="/showTours/bus" class="btn btn-outline-primary "><spring:message code="bus.tour"/></a>\n' +
                            '                <a href="/showTours/air" class="btn btn-outline-primary "><spring:message code="air.tour"/></a>\n');

                        $('#user').append('<button class="btn btn-outline-primary dropdown-toggle position-button" type="button" data-toggle="dropdown">${pageContext.request.userPrincipal.name}<span class="glyphicon glyphicon-user"/>\n' +
